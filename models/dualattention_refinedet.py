...
def add_tcb(cfg):
    ...
    feature_pred_layers = []
    spatial_attention_layers1 = []
    spatial_attention_layers2 = []
    spatial_attention_layers3 = []
    for k, v in enumerate(cfg):
        ...
        spatial_attention_layers1 += [nn.Conv2d(256, 256, 3, padding=1),
                                    nn.BatchNorm2d(256),
                                    nn.ReLU(inplace=True)]
        spatial_attention_layers2 += [nn.Conv2d(256, 256, 3, padding=1),
                                    nn.BatchNorm2d(256),
                                    nn.ReLU(inplace=True)]
        spatial_attention_layers3 += [nn.Conv2d(256, 256, 3, padding=1),
                                    nn.BatchNorm2d(256),
                                    nn.ReLU(inplace=True)]

        if k != len(cfg) - 1:
            feature_upsample_layers += [nn.ConvTranspose2d(256, 256, 2, 2)]
    return (feature_scale_layers, feature_upsample_layers, feature_pred_layers, spatial_attention_layers1, spatial_attention_layers2, spatial_attention_layers3)
