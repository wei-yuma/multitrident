...
class branch_for_middle(nn.Module):

    def __init__(self, inplanes, planes):
        super(branch_for_middle, self).__init__()
        ...
        self.share_weight4conv5 = nn.Parameter(torch.randn(planes, planes, 3, 3))
        ...
        self.share_weight4conv11 = nn.Parameter(torch.randn(planes, planes, 3, 3))
    ...
    def forward_once_for_middle(self, x):
        ...
        out = nn.functional.conv2d(out, self.share_weight4conv3, stride=1, padding=3, bias=self.bias3, dilation=3)
        out = self.bn3(out)
        out = self.relu(out)
        ...
        out = nn.functional.conv2d(out, self.share_weight4conv5, stride=1, padding=1, bias=self.bias5, dilation=1)
        out = self.bn5(out)
        out = self.relu(out)

        out = nn.functional.conv2d(out, self.share_weight4conv6, stride=1, padding=3, bias=self.bias6, dilation=3)
        out = self.bn6(out)
        out = self.relu(out)
        ...
        out = nn.functional.conv2d(out, self.share_weight4conv9, stride=1, padding=3, bias=self.bias9, dilation=3)
        out = self.bn9(out)
        out = self.relu(out)
        ...
        out = nn.functional.conv2d(out, self.share_weight4conv11, stride=1, padding=1, bias=self.bias11, dilation=1)
        out = self.bn11(out)
        out = self.relu(out)

        out = nn.functional.conv2d(out, self.share_weight4conv12, stride=1, padding=3, bias=self.bias12, dilation=3)
        out = self.bn12(out)
        out = self.relu(out)

        return out

    def forward(self, input_for_middle):
